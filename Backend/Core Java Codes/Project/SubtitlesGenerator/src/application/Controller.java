...
public class Controller {
	@FXML private Label urlMessageLabel;				// Label that shows error message in audio URL tab.
	@FXML private Label fileMessageLabel;				// Label that shows error message in File Upload tab.
	@FXML private TextField urlField;					// User can enter Audio URL here.
	@FXML private Button makeSubsFromURL;				// Starts the subtitle making process in audio URL tab.
	@FXML private Button makeSubsFromFile;				// Starts the subtitle making process in file upload tab.
	
	
	private final String apiUrl = "https://api.assemblyai.com/v2/";						// API URL
	private final String transcriptString = "transcript", uploadString = "upload";		// ENDPOINTS to tell if we are uploading a file or making subs.
	...
	private File SelectFile()
	{
		...
		selectedFile.setReadable(true);	selectedFile.setWritable(true);
	...
	private UploadURL CreateFileUploadLink()
	{
		...
		try {
			HttpRequest request = HttpRequest.newBuilder()
					.uri(new URI(apiUrl+uploadString))
					.header("Authorization", apiKey)
					.header("Transer-Encoding", "chunked")
					.POST(BodyPublishers.ofFile(selectedFile.toPath()))
					.build();
	...
	private void Fetch(String url, Label messageLabel)
	{
		...
		try {
			HttpRequest request = HttpRequest.newBuilder()
									.uri(new URI(apiUrl + transcriptString))
									.header("Authorization", apiKey)
			...
			request = HttpRequest.newBuilder()
						.uri(new URI(apiUrl + transcriptString + "/" + result.getId()))
						.header("Authorization", apiKey)
						.GET()
						.build();
...
//		Change the file extention back to srt.
	...
	public void SetExtentionFilter(FileChooser fileChooser, String extension)
	{
		FileChooser.ExtensionFilter extensionFilter = 
				new FileChooser.ExtensionFilter("FILE EXTENTION : (*." +extension + ")", "*." +extension);
